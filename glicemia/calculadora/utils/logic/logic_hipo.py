from ..constants import LIMITE_SUSPENDER_INFUSION, UMBRAL_HIPER, UMBRAL_HIPO
from ..helpers import (
    _a_bool,
    _a_decimal,
...
def evaluar_hipoglucemia(actual, previa=None, infusion_activa=False):
    ...
    if previa is not None and previa <= UMBRAL_HIPO and actual >= UMBRAL_HIPO:
        ...
        # 121 a 179
        if LIMITE_SUSPENDER_INFUSION < actual < UMBRAL_HIPER:
            resultado["estado"] = "Recontrol Post-Hipoglucemia"
            ...
            resultado["conducta"] = (
                "Continuar monitoreo estrecho. No reiniciar insulina de inmediato."
            )
            resultado["suspender_insulina"] = infusion_activa
            resultado["proximo_control"] = control["proximo_control"]
            resultado["comentario_control"] = control["comentario_control"]
        ...
        if actual >= UMBRAL_HIPER:
            ...
            resultado["resumen_objetivo"] = "Fuera de rango objetivo"
            resultado["conducta"] = (
                "Reevaluar reinicio de insulina según evolución clínica y contexto."
            )
